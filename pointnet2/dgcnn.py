...
from pytorch_utils import AlwaysOnDropout
...
class DGCNNSeg(nn.Module):
    def __init__(self, edgeconv_widths, dgcnn_mlp_widths, pc_in_dim, dgcnn_k, num_classes, mc_drop=False):
        super(DGCNNSeg, self).__init__()
        self.encoder = DGCNN(edgeconv_widths, dgcnn_mlp_widths, pc_in_dim, k=dgcnn_k, return_edgeconvs=True)
        in_dim = dgcnn_mlp_widths[-1]
        for edgeconv_width in edgeconv_widths:
            in_dim += edgeconv_width[-1]
        if mc_drop:
...
if __name__ == "__main__":
    model = DGCNNSeg([[64,64], [64, 64], [64, 64]], [512, 256], 9, 20, 20)
    model.cuda()
    inp = torch.randn(16, 2048, 9).cuda()
    res = model(inp)
    print(res.shape)