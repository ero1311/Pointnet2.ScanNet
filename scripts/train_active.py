...
from eval import forward, filter_points, compute_acc, compute_miou, eval_one_batch, eval_wholescene
...
def mc_forward(args, model, coords, feats, mc_iters=10):
    pred = []
    ...
    for coord, feat in zip(coord_chunk, feat_chunk):
        ...
        for _ in range(mc_iters):
            outputs.append(model(torch.cat([coord, feat], dim=2)).detach().cpu())
        pred.append(sum(outputs) / mc_iters)
...
def choose_uncertain_scenes(args, scene_list, stamp, num_scenes=100):
    ...
    for scene in scene_list:
        ...
        preds = preds.view(-1, C).cpu().numpy()
        ...
        scene_entropy = -1 * preds * np.log(preds)
        scene_entropy = scene_entropy.sum(axis=1).mean()
        scene_uncertainties[scene] = scene_entropy
    
    return list(OrderedDict(sorted(scene_uncertainties.items(), key=lambda x: x[1], reverse=True)).keys())[:num_scenes]
...
def run_experiments(args):
    ...
    if len(experiments) == 0:
        ...
        max_id = max(experiments, key=lambda x: int(x.split("_")[1]))
    ...
    for i in range(1, 6):
        ...
        new_scenes = choose_uncertain_scenes(args, mc_scenes_pool, prev_mc_experiment)
